...
def postprocess_ppo_gae(
    ...
    batches = []
    a_w = 2
    for i in ['0', '1', '2', '3']: #, '3', '4', '5']: #samplebatch_infos_rewards.keys():
        ...
        sample_batch_agent[SampleBatch.ACTIONS] = sample_batch[SampleBatch.ACTIONS][:, a_w*int(i):a_w*(int(i)+1)]
        ...
        if not np.all(np.isfinite(sample_batch_agent[SampleBatch.REWARDS])):
            import pdb; pdb.set_trace()
        if not np.all(np.isfinite(sample_batch_agent[SampleBatch.ACTIONS])):
            import pdb; pdb.set_trace()
        if not np.all(np.isfinite(sample_batch_agent[SampleBatch.VF_PREDS])):
            import pdb; pdb.set_trace()
        if not np.all(np.isfinite(sample_batch_agent[Postprocessing.ADVANTAGES])):
            import pdb; pdb.set_trace()
        if not np.all(np.isfinite(sample_batch_agent[Postprocessing.VALUE_TARGETS])):
            import pdb; pdb.set_trace()
...
def ppo_surrogate_loss(
    ...
    Returns:
        Union[TensorType, List[TensorType]]: A single loss tensor or a list
            of loss tensors.
    """
    #if Postprocessing.ADVANTAGES not in train_batch.keys() and Postprocessing.VALUE_TARGETS not in train_batch.keys():
    #    return None
    ...
    def breakpoint():
        torch.save(train_batch[SampleBatch.OBS], "m_obs.pkl")
        #torch.save(train_batch[SampleBatch.CUR_OBS], "m_cur_obs.pkl")
        #torch.save(train_batch[SampleBatch.NEXT_OBS], "m_next_obs.pkl")
        torch.save(logits, "logits.pkl")
        torch.save(train_batch[SampleBatch.ACTIONS], "train_batch_actions.pkl")
        torch.save(train_batch[SampleBatch.ACTION_LOGP], "train_batch_action_logp.pkl")
        torch.save(logps, "logp.pkl")
        import pdb; pdb.set_trace()
    
    #print("Actions", train_batch[SampleBatch.ACTIONS].mean(dim=0), train_batch[SampleBatch.ACTIONS].max(dim=0))
    if not torch.all(torch.isfinite(logps)):
        breakpoint()
    if not torch.all(torch.isfinite(entropies)):
        breakpoint()
